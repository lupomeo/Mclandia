...
void
olc_interpreter (void *targ, int mode, char *arg)
{
	int error = 0, command;
...
int
can_modify (struct char_data *ch, int vnum)
{
	return (1);
}
...
void
olc_string (char **string, size_t maxlen, char *arg)
{
	skip_spaces (&arg);
...
void
olc_bitvector (int *bv, const char **names, char *arg)
{
...
void
olc_set_show (struct char_data *ch, int olc_mode, char *arg)
{
}
