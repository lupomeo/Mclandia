...
/* local functions */
int read_type_list (FILE * shop_f, struct shop_buy_data *list, int new_format,
					int max);
...
is_ok_char (struct char_data *keeper, struct char_data *ch, int shop_nr)
{
	...
	if ((IS_MAGIC_USER (ch) && NOTRADE_MAGIC_USER (shop_nr)) ||
		...
		(IS_MARTIAL (ch) && NOTRADE_MARTIAL (shop_nr)) ||
		(IS_LINKER (ch) && NOTRADE_LINKER (shop_nr)) ||
		(IS_PSIONIC (ch) && NOTRADE_PSIONICO (shop_nr)) ||
...
pop (struct stack_data *stack)
{
	...
	{
		log ("SYSERR: Illegal expression %d in shop keyword list.",
			 S_LEN (stack));
		return (0);
	}
}
		...
		else
		{
			if ((temp = find_oper_num (*ptr)) == NOTHING)
			{
				...
				for (index = 0; *extra_bits[index] != '\n'; index++)
					...
					{
						push (&vals,
							  IS_SET (GET_OBJ_EXTRA (obj), 1 << index));
						break;
					}
				if (*extra_bits[index] == '\n')
		...
		if (SHOP_BUYTYPE (shop_nr, counter) == GET_OBJ_TYPE (item))
		{
			if ((GET_OBJ_VAL (item, 2) == 0) &&
				...
				if (evaluate_expression
					(item, SHOP_BUYWORD (shop_nr, counter)))
				return (OBJECT_OK);
		}
	return (OBJECT_NOTOK);
}
...
int
buy_price (struct obj_data *obj, int shop_nr)
{
			  ...
			  struct char_data *keeper, int shop_nr)
{
	...
	{
		sprintf (buf, "%s Un valere negativo?  Cerchi di vendermi qualcosa.",
				 GET_NAME (ch));
		do_tell (keeper, buf, cmd_tell, 0);
		return;
	}
	if (!(*arg) || !(buynum))
	{
		sprintf (buf, "%s Cosa vuoi comperare??", GET_NAME (ch));
		do_tell (keeper, buf, cmd_tell, 0);
		return;
	}
	...
	{
		sprintf (buf, "%s: Non puoi portare altri oggetti.\r\n",
				 fname (obj->name));
		send_to_char (buf, ch);
		return;
	}
	...
	{
		sprintf (buf, "%s: Non riesci a portare altro peso.\r\n",
				 fname (obj->name));
		send_to_char (buf, ch);
		return;
	}
	...
	if (bought < buynum)
	{
		if (!obj || !same_obj (last_obj, obj))
			sprintf (buf, "%s Ho solo %d da venderti.", GET_NAME (ch),
					 bought);
		else if (GET_GOLD (ch) < buy_price (obj, shop_nr))
			sprintf (buf, "%s Puoi comperare solo %d.", GET_NAME (ch),
					 bought);
		else if (IS_CARRYING_N (ch) >= CAN_CARRY_N (ch))
			sprintf (buf, "%s Puoi tenere solo %d.", GET_NAME (ch), bought);
		else if (IS_CARRYING_W (ch) + GET_OBJ_WEIGHT (obj) > CAN_CARRY_W (ch))
			sprintf (buf, "%s Puoi portare solo %d.", GET_NAME (ch), bought);
		else
			sprintf (buf, "%s Solo un pazzo puo' darti %d.", GET_NAME (ch),
					 bought);
		do_tell (keeper, buf, cmd_tell, 0);
	}
	if (!IS_GOD (ch))
				 ...
				 struct char_data *keeper, int shop_nr, int msg)
{
	...
	{
	case OBJECT_NOVAL:
		sprintf (buf, "%s Stai scherzando? Questo affare e' inutile!",
				 GET_NAME (ch));
		break;
	case OBJECT_NOTOK:
...
int
sell_price (struct char_data *ch, struct obj_data *obj, int shop_nr)
{
			   ...
			   struct char_data *keeper, int shop_nr)
{
	...
	{
		sprintf (buf, "%s Un valore negativo?  Prova a comprare qualcosa.",
				 GET_NAME (ch));
		do_tell (keeper, buf, cmd_tell, 0);
		return;
	}
	...
	{
		if (!obj)
			sprintf (buf, "%s Hai solo %d di questi.", GET_NAME (ch), sold);
		else if (GET_GOLD (keeper) + SHOP_BANK (shop_nr) <
				 sell_price (ch, obj, shop_nr))
			sprintf (buf, "%s Posso permettermi di comperare %d di questi.",
					 GET_NAME (ch), sold);
		else
			sprintf (buf, "%s Solo un pazzo puo comperare %d di questi.",
					 GET_NAME (ch), sold);
	...
	do_tell (keeper, buf, cmd_tell, 0);
	sprintf (buf, "Adesso il negoziante ha %s.\r\n", tempstr);
	send_to_char (buf, ch);

	if (GET_GOLD (keeper) < MIN_OUTSIDE_BANK)
	{
		goldamt =
			MIN (MAX_OUTSIDE_BANK - GET_GOLD (keeper), SHOP_BANK (shop_nr));
				...
				struct char_data *keeper, int shop_nr)
{
	...
	if (!(*arg))
	{
		sprintf (buf, "%s Cosa e come lo valuto??", GET_NAME (ch));
		do_tell (keeper, buf, cmd_tell, 0);
		return;
	}
	one_argument (arg, name);
	...
	sprintf (buf, "%s Ti daro' %d monete d'oro per questo!", GET_NAME (ch),
			 sell_price (ch, obj, shop_nr));
	do_tell (keeper, buf, cmd_tell, 0);
...
char *
list_object (struct obj_data *obj, int cnt, int index, int shop_nr)
{
	static char buf[256];
			...
			if (CAN_SEE_OBJ (ch, obj) && (obj->obj_flags.cost > 0))
			{
				...
				else
				{
					...
					if (!(*name) || isname (name, last_obj->name))
						strcat (buf,
								list_object (last_obj, cnt, index, shop_nr));
					cnt = 1;
					last_obj = obj;
				}
			}
...
SPECIAL (shop_keeper)
{
	...
	if (CMD_IS ("steal"))
	{
		sprintf (argm, "$N urla '%s'", MSG_NO_STEAL_HERE);
...
{
	if (error)
		log ("SYSERR: Raise MAX_SHOP_OBJ constant in shop.h to %d",
			 len + error);
	BUY_WORD (list[len]) = 0;
...
void
read_line (FILE * shop_f, const char *string, void *data)
{
				...
				int new_format, int max)
{
	...
	do
	{
		...
		for (index = 0, num = NOTHING; *item_types[index] != '\n'; index++)
			if (!strn_cmp
				(item_types[index], buf, strlen (item_types[index])))
			{
				num = index;
...
char *
read_shop_message (int mnum, room_vnum shr, FILE * shop_f, const char *why)
{
	...
	for (cht = 0; tbuf[cht]; cht++)
	{
		...
		else if (tbuf[cht + 1] == 'd' && (mnum == 5 || mnum == 6))
		{
			if (ss == 0)
			{
				log ("SYSERR: Shop #%d has %%d before %%s, message #%d.", shr,
					 mnum);
				err++;
			}
			ds++;
		...
		{
			log ("SYSERR: Shop #%d has invalid format '%%%c' in message #%d.",
				 shr, tbuf[cht + 1], mnum);
			err++;
		}
	}
	...
	{
		log
			("SYSERR: Shop #%d has too many specifiers for message #%d. %%s=%d %%d=%d",
			 shr, mnum, ss, ds);
		err++;
	}
...
char *
customer_string (int shop_nr, int detailed)
{
	int index, cnt = 1;
	static char buf[256];
...
void
list_all_shops (struct char_data *ch)
{
	int shop_nr;
...
void
list_detailed_shop (struct char_data *ch, int shop_nr)
{
	struct obj_data *obj;
	struct char_data *k;
	int index, temp;
	...
	for (index = 0; SHOP_ROOM (shop_nr, index) != NOWHERE; index++)
	{
		...
		if ((temp = real_room (SHOP_ROOM (shop_nr, index))) != NOWHERE)
			sprintf (buf1, "%s (#%d)", world[temp].name,
					 GET_ROOM_VNUM (temp));
		else
	...
	sprintf (buf,
			 "Buy at:     [%4.2f], Sell at: [%4.2f], Open: [%d-%d, %d-%d]%s",
			 SHOP_SELLPROFIT (shop_nr), SHOP_BUYPROFIT (shop_nr),
