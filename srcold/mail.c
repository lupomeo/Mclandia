...
find_char_in_index (long searchee)
{
	...
	for (tmp = mail_index; (tmp && tmp->recipient != searchee);
		 tmp = tmp->next);
...
write_to_file (void *buf, int size, long filepos)
{
	...
	{
		log
			("SYSERR: Mail system -- fatal error #2!!! (invalid file position %ld)",
			 filepos);
		no_mail = TRUE;
		return;
	}
...
read_from_file (void *buf, int size, long filepos)
{
	...
	{
		log
			("SYSERR: Mail system -- fatal error #3!!! (invalid filepos read %ld)",
			 filepos);
		no_mail = TRUE;
		return;
	}
...
index_mail (long id_to_index, long pos)
{
	...
	{
		log
			("SYSERR: Mail system -- non-fatal error #4. (id_to_index == %ld)",
			 id_to_index);
		return;
	}
