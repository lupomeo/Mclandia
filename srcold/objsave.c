...
struct obj_data *
Obj_from_store (struct obj_file_elem object, int *location)
{
	struct obj_data *obj;
	int j;
...
int
Obj_to_store (struct obj_data *obj, FILE * fl, int location)
{
	int j;
...
void
auto_equip (struct char_data *ch, struct obj_data *obj, int location)
{
	int j;
...
Crash_delete_file (char *name)
{
	...
	if (remove (filename) < 0 && errno != ENOENT)
		log ("SYSERR: deleting crash file %s (2): %s", filename,
			 strerror (errno));
...
Crash_clean_file (char *name)
{
	...
	{
		if (errno != ENOENT)	/* if it fails, NOT because of no file */
			log ("SYSERR: OPENING OBJECT FILE %s (4): %s", fname,
				 strerror (errno));
		return (0);
	}
	if (!feof (fl))
...
Crash_load (struct char_data *ch)
{
	...
	if (!(fl = fopen (fname, "r+b")))
	{
		if (errno != ENOENT)
		{						/* if it fails, NOT because of no file */
			log ("SYSERR: READING OBJECT FILE %s (5): %s", fname,
				 strerror (errno));
			send_to_char
				("\r\n********************* NOTICE *********************\r\n"
...
int
Crash_save (struct obj_data *obj, FILE * fp, int location)
{
	struct obj_data *tmp;
	int result;
 ...
 */
void
Crash_extract_norent_eq (struct char_data *ch)
{
	int j;
...
Crash_rent_deadline (struct char_data *ch, struct char_data *recep, long cost)
{
	...
	sprintf (buf,
			 "$n ti dice, 'Puoi affittare una stanza per %ld day%s \r\n"
			 "con i soldi che hai appresso e in banca.'\r\n",
						  ...
						  struct obj_data *obj)
{
	...
	if (obj)
	{
		...
		{
			has_norents = 1;
			sprintf (buf, "$n ti dice, 'Non puoi depositare %s.'",
					 OBJS (obj, ch));
			act (buf, FALSE, recep, 0, ch, TO_VICT);
		}
		has_norents += Crash_report_unrentables (ch, recep, obj->contains);
		has_norents +=
			Crash_report_unrentables (ch, recep, obj->next_content);
	}
	return (has_norents);
}
	...
	if (obj)
	{
		if (!Crash_is_unrentable (obj))
		{
			...
			*cost += MAX (0, (GET_OBJ_RENT (obj) * factor));
			if (display)
			{
				sprintf (buf, "$n ti dice, '%d coins per %s..'",
		...
		Crash_report_rent (ch, recep, obj->contains, cost, nitems, display,
						   factor);
		Crash_report_rent (ch, recep, obj->next_content, cost, nitems,
						   display, factor);
	}
}
				  ...
				  int display, int factor)
{
	...
	if (!numitems)
	{
		act ("$n ti dice, 'Ma tu non hai niente!  Fai semplicemente quit!'",
	...
	{
		sprintf (buf,
				 "$n ti dice, 'Spiacente ma io non posso conservare piu' di %d oggetti.'",
				 max_obj_save);
	...
	if (display)
	{
		sprintf (buf, "$n ti dice, 'Piu' la mia tassa di %d monete..'",
				 min_rent_cost * factor);
		act (buf, FALSE, receptionist, 0, ch, TO_VICT);
		sprintf (buf, "$n ti dice, 'Per un totale di %ld coins%s.'",
				 totalcost, (factor == RENT_FACTOR ? " al giorno" : ""));
		act (buf, FALSE, receptionist, 0, ch, TO_VICT);
		if (totalcost > GET_GOLD (ch) + GET_BANK_GOLD (ch))
		{
			act ("$n ti dice, '...che mi sembra che tu non puoi sostenere.'",
				 FALSE, receptionist, 0, ch, TO_VICT);
			return (0);
		}
				  ...
				  int cmd, char *arg, int mode)
{
	...
	if (!AWAKE (recep))
	{
		sprintf (buf, "%s non e' in grado di parlare con te...\r\n",
				 HSSH (recep));
		send_to_char (buf, ch);
		return (TRUE);
	}
	if (!CAN_SEE (recep, ch))
	{
		act ("$n dice, 'Non tratto con gente che non posso vedere!'", FALSE,
			 recep, 0, 0, TO_ROOM);
		return (TRUE);
	}
	if (free_rent)
	{
		act
			("$n ti dice, 'L'affitto e' gratis oggi.  quit e i tuoi oggetti saranno salvati!'",
	...
	if (CMD_IS ("rent"))
	{
		...
		if (mode == RENT_FACTOR)
			sprintf (buf,
					 "$n ti dice, 'L'affitto ti costa %d monete d'oro al giorno.'",
					 cost);
		else if (mode == CRYO_FACTOR)
			sprintf (buf,
					 "$n ti dice, 'ti costera' %d gold coins essere congelato.'",
					 cost);
		act (buf, FALSE, recep, 0, ch, TO_VICT);
		if (cost > GET_GOLD (ch) + GET_BANK_GOLD (ch))
		{
			act ("$n ti dice, '...che mi sembra tu non puoi affrontare.'",
				 FALSE, recep, 0, ch, TO_VICT);
			return (TRUE);
		}
		...
		{
			act
				("$n ripone i tuoi oggetti in cassaforte e ti accompagna nella tua stanza.",
				 FALSE, recep, 0, ch, TO_VICT);
		...
		{						/* cryo */
			act
				("$n ripone i tuoi oggetti in cassaforte e ti accompagna nella tua stanza.\r\n"
				 ...
				 "You begin to lose consciousness...", FALSE, recep, 0, ch,
				 TO_VICT);
