...
void
write_aliases (struct char_data *ch)
{
	FILE *file;
	...
	{
		log ("SYSERR: Couldn't save aliases for %s in '%s'.", GET_NAME (ch),
			 fn);
	...
	for (temp = GET_ALIASES (ch); temp; temp = temp->next)
	{
		...
		fprintf (file, "%d\n%s\n"	/* Alias */
				 ...
				 "%d\n",		/* Type */
				 aliaslen, temp->alias,
				 repllen, temp->replacement + 1, temp->type);
	}
...
void
read_aliases (struct char_data *ch)
{
	FILE *file;
	...
	if ((file = fopen (xbuf, "r")) == NULL)
	{
		if (errno != ENOENT)
		{
			log ("SYSERR: Couldn't open alias file '%s' for %s.", xbuf,
				 GET_NAME (ch));
