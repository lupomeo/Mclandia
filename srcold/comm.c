...
main (int argc, char **argv)
{
	...
	while ((pos < argc) && (*(argv[pos]) == '-'))
	{
		...
		case 'o':
			...
			{
				puts
					("SYSERR: File name to log to expected after option -o.");
				exit (1);
			}
			break;
		case 'd':
    ...
    sprintf(buf, "----- Mclandia release build %d -----",BUILD_VERSION);
    log(buf);
 ...
 */
#ifdef HAS_RLIMIT
	{
		...
		if (limit.rlim_max == RLIM_INFINITY)
			...
			max_descs =
				MIN (max_playing + NUM_RESERVED_DESCS, limit.rlim_max);
#else
 ...
 */
void
game_loop (socket_t mother_desc)
{
	...
	while (!circle_shutdown)
	{
		...
		if (descriptor_list == NULL)
		{
			...
			if (select
				(mother_desc + 1, &input_set, (fd_set *) 0, (fd_set *) 0,
				 NULL) < 0)
			{
				if (errno == EINTR)
		...
		if (select
			(maxdesc + 1, &input_set, &output_set, &exc_set, &null_time) < 0)
		{
		...
		{
			log
				("SYSERR: **BAD** MISSED_PULSES NONPOSITIVE (%d), TIME GOING BACKWARDS!!",
				 missed_pulses);
			missed_pulses = 1;
		}
...
/* if (!(pulse % (1 * PASSES_PER_SEC))) *//* for debug purpose */
	{
		weather_and_time (1);
		affect_update ();
		point_update ();
 ...
 */
void
timediff (struct timeval *rslt, struct timeval *a, struct timeval *b)
{
 ...
 */
void
timeadd (struct timeval *rslt, struct timeval *a, struct timeval *b)
{
	rslt->tv_sec = a->tv_sec + b->tv_sec;
	rslt->tv_usec = a->tv_usec + b->tv_usec;
	...
	else if (STATE (d) == CON_PLAYING && !IS_NPC (d->character))
	{
		...
		*prompt = '\0';
		...
		if (PLR_FLAGGED (d->character, PLR_CRIMINAL))
			count += sprintf (prompt + count, "[Jailed] ");

		if (GET_INVIS_LEV (d->character))
			count +=
				sprintf (prompt + count, "i%d ",
		...
		if (PRF_FLAGGED (d->character, PRF_DISPHP))
		{
			...
			{
				count += sprintf (prompt + count, "\x1B[0;32mH\x1B[0;0m");
				...
				if (percent >= 100)
					count +=
						sprintf (prompt + count, "\x1B[0;32m%d\x1B[0;0m",
								 GET_HIT (d->character));
				else if (percent >= 90)
					count +=
						sprintf (prompt + count, "\x1B[0;32m%d\x1B[0;0m",
					...
					count +=
						sprintf (prompt + count, "\x1B[0;0m%d\x1B[0;0m",
								 GET_HIT (d->character));
				else if (percent >= 30)
					count +=
						sprintf (prompt + count, "\x1B[0;0m%d\x1B[0;0m ",
					...
					count +=
						sprintf (prompt + count, "\x1B[1;31m%d\x1B[0;0m",
								 GET_HIT (d->character));
				else if (percent >= 0)
					count +=
						sprintf (prompt + count, "\x1B[1;31m%d\x1B[0;0m",
			...
			{
				count +=
					sprintf (prompt + count, "%dH ", GET_HIT (d->character));
			}
		}
		...
		if (PRF_FLAGGED (d->character, PRF_DISPMANA))
		{
			if (COLOR_LEV (d->character) >= C_CMP)
			{
				count += sprintf (prompt + count, "\x1B[1;36mR\x1B[0;0m");
				...
				if (percent >= 100)
					count +=
						sprintf (prompt + count, "\x1B[1;36m%d\x1B[0;0m",
					...
					count +=
						sprintf (prompt + count, "\x1B[0;36m%d\x1B[0;0m",
								 GET_MANA (d->character));
				else if (percent >= 30)
					count +=
						sprintf (prompt + count, "\x1B[0;36m%d\x1B[0;0m",
					...
					count +=
						sprintf (prompt + count, "\x1B[1;35m%d\x1B[0;0m",
								 GET_MANA (d->character));
				else if (percent >= 0)
					count +=
						sprintf (prompt + count, "\x1B[1;35m%d\x1B[0;0m",
			...
			{
				count +=
					sprintf (prompt + count, "%dR ", GET_MANA (d->character));
			}
		}
		...
		if (PRF_FLAGGED (d->character, PRF_DISPMOVE))
		{
			if (COLOR_LEV (d->character) >= C_CMP)
			{
				count += sprintf (prompt + count, "\x1B[0;33mV\x1B[0;0m");
				...
				if (percent >= 100)
					count +=
						sprintf (prompt + count, "\x1B[0;33m%d\x1B[0;0m",
			...
			{
				count +=
					sprintf (prompt + count, "%dV ", GET_MOVE (d->character));
			}
		}
		...
		if (PRF_FLAGGED (d->character, PRF_DISPGOLD))
		{
			if (COLOR_LEV (d->character) >= C_CMP)
				count +=
					sprintf (prompt + count, "\x1B[1;33m%dG\x1B[0;0m ",
							 GET_GOLD (d->character));
			else
				count +=
					sprintf (prompt + count, "%dG ", GET_GOLD (d->character));
		}
		...
		if (GET_LEVEL (d->character) < LVL_IMMORT - 1)
		{
			if (PRF_FLAGGED (d->character, PRF_DISPEXP))
			{
				...
				{
					count +=
						sprintf (prompt + count, "\x1B[0;36m%dX\x1B[0;0m ",
								 level_exp (GET_CLASS (d->character),
											GET_LEVEL (d->character) + 1) -
								 GET_EXP (d->character));
				}
				else
				{
					count += sprintf (prompt + count, "%dX ",
									  level_exp (GET_CLASS (d->character),
												 GET_LEVEL (d->character) +
												 1) - GET_EXP (d->character));
				}
			}
		}
		if (GET_LEVEL (d->character) >= LVL_IMMORT - 1)
		{
			if (PRF_FLAGGED (d->character, PRF_DISPEXP))
			{
				if (COLOR_LEV (d->character) >= C_CMP)
					count +=
						sprintf (prompt + count, "\x1B[0;36m0X\x1B[0;0m ");
				else
					count += sprintf (prompt + count, "0X ");
			}
		}
		...
		if (PRF_FLAGGED (d->character, PRF_DISPEXITS))
		{
			...
			for (door = 0; door < NUM_OF_DIRS; door++)
			{
				if (EXIT (d->character, door)
					...
					count +=
						sprintf (prompt + count, "%c ", LOWER (*dirs[door]));
			}
			count += sprintf (prompt + count, "] ");
		}
		...
		if (FIGHTING (d->character) != NULL)
		{
			...
			if (FIGHTING (FIGHTING (d->character)) != NULL)
			{
				...
				if (GET_MAX_HIT (FIGHTING (FIGHTING (d->character))) > 0)
					percent =
						(100 * GET_HIT (FIGHTING (FIGHTING (d->character))) /
						 GET_MAX_HIT (FIGHTING (FIGHTING (d->character))));
				else
					percent = -1;

				if (percent >= 100)
					sprintf (prompt + strlen (prompt), "Eccellente/");
				else if (percent >= 90)
					sprintf (prompt + strlen (prompt), "Contuso/");
				else if (percent >= 75)
					sprintf (prompt + strlen (prompt), "Graffiato/");
				else if (percent >= 50)
					sprintf (prompt + strlen (prompt), "Tagliato/");
				else if (percent >= 30)
					sprintf (prompt + strlen (prompt), "Ferito/");
				else if (percent >= 15)
					sprintf (prompt + strlen (prompt), "Sanguinante/");
				else if (percent >= 0)
					sprintf (prompt + strlen (prompt), "Squarciato/");
				else
					sprintf (prompt + strlen (prompt), "Tramortito/");
			}

/* Uncomment se vogliamo il nome del mob
			if (COLOR_LEV(d->character) >= C_CMP) 
			{
				sprintf(prompt + strlen(prompt), "%s ", GET_NAME(FIGHTING(d->character)));
			} 
			else 
			{
				sprintf(prompt + strlen(prompt), "%s ", GET_NAME(FIGHTING(d->character))); 
...
*/
			if (COLOR_LEV (d->character) >= C_CMP)
			{
				...
				if (GET_MAX_HIT (FIGHTING (d->character)) > 0)
					percent =
						(100 * GET_HIT (FIGHTING (d->character))) /
...
/*				
				sprintf(prompt + strlen(prompt), "(%d%%) ", 
                    (int)(GET_HIT(FIGHTING(d->character))*100)/GET_MAX_HIT(FIGHTING(d->character)));
*/
				sprintf (prompt + strlen (prompt), "M:");
				if (percent >= 100)
					sprintf (prompt + strlen (prompt),
							 "\x1B[0;32mEccellente\x1B[0;0m");
				else if (percent >= 90)
					sprintf (prompt + strlen (prompt),
							 "\x1B[0;32mContuso\x1B[0;0m");
				else if (percent >= 75)
					sprintf (prompt + strlen (prompt),
							 "\x1B[0;32mGraffiato\x1B[0;0m");
				else if (percent >= 50)
					sprintf (prompt + strlen (prompt),
							 "\x1B[0;0mTagliato\x1B[0;0m");
				else if (percent >= 30)
					sprintf (prompt + strlen (prompt),
							 "\x1B[0;0mFerito\x1B[0;0m");
				else if (percent >= 15)
					sprintf (prompt + strlen (prompt),
							 "\x1B[1;31mSanguinante\x1B[0;0m");
				else if (percent >= 0)
					sprintf (prompt + strlen (prompt),
							 "\x1B[1;31mSquarciato\x1B[0;0m");
				else
					sprintf (prompt + strlen (prompt),
							 "\x1B[1;31mTramortito\x1B[0;0m");
...
void
write_to_q (const char *txt, struct txt_q *queue, int aliased)
{
 ...
 */
int
parse_ip (const char *addr, struct in_addr *inaddr)
{
...
set_sendbuf (socket_t s)
{
	...
	if (setsockopt (s, SOL_SOCKET, SO_SNDBUF, (char *) &opt, sizeof (opt)) <
		0)
	{
 ...
 */
int
write_to_descriptor (socket_t desc, const char *txt)
{
	size_t total;
 ...
 */
ssize_t
perform_socket_read (socket_t desc, char *read_point, size_t space_left)
{
	ssize_t ret;
...
process_input (struct descriptor_data *t)
{
	...
	do
	{
		...
		bytes_read =
			perform_socket_read (t->descriptor, read_point, space_left);
		...
		*write_point = '\0';
		...
		else if (*tmp == '!' && *(tmp + 1))
		{
			...
			int starting_pos = t->history_pos,
				cnt =
				(t->history_pos == 0 ? HISTORY_SIZE - 1 : t->history_pos - 1);
	...
	*(second++) = '\0';
	...
	if (((strpos - orig) + strlen (first)) < strlen (orig))
		strncat (newsub, strpos + strlen (first),
				 (MAX_INPUT_LENGTH - strlen (newsub) - 1));
...
close_socket (struct descriptor_data *d)
{
	...
	if (d->character)
	{
		...
		if (STATE (d) == CON_PLAYING || STATE (d) == CON_DISCONNECT)
		{
			...
			act ("$n has lost $s link.", TRUE, link_challenged, 0, 0,
				 TO_ROOM);

			/* Modifica per salvare gli oggetti in caso di link-dead */
			/* Crash_crashsave (link_challenged); */ /* rimosso  perche non risolveva un cazzo */
			/* fine modifica */
			save_char (link_challenged, NOWHERE);

			sprintf (buf, "Closing link to: %s.", GET_NAME (link_challenged));
			mudlog (buf, NRM,
					MAX (LVL_IMMORT, GET_INVIS_LEV (link_challenged)), TRUE);
			link_challenged->desc = NULL;
		}
		else
		{
			sprintf (buf, "Losing player: %s.",
					 GET_NAME (d->character) ? GET_NAME (d->character) :
					 "<null>");
...
void
send_to_outdoor (const char *messg)
{
	struct descriptor_data *i;
...
void
send_to_zone (const char *messg, zone_vnum zone)
{
	struct descriptor_data *i;
	...
	for (i = descriptor_list; i; i = i->next)
	{
		...
		if (!AWAKE (i->character) || IN_ZONE (i->character) != zone)
		{
			continue;
		}
		SEND_TO_Q (messg, i);
...
void
send_to_sector (const char *messg, int sector)
{
	struct descriptor_data *i;
	...
	for (i = descriptor_list; i; i = i->next)
	{
		...
		if (!AWAKE (i->character) || IN_SECTOR (i->character) != sector)
		{
			continue;
		}
		SEND_TO_Q (messg, i);
...
void
send_to_room (const char *messg, room_rnum room)
{
	struct char_data *i;
			 ...
			 const void *vict_obj, const struct char_data *to)
{
	...
	for (;;)
	{
		if (*orig == '$')
		{
			...
			case 'N':
				CHECK_NULL (vict_obj,
							PERS ((const struct char_data *) vict_obj, to));
				break;
			case 'm':
				i = HMHR (ch);
				break;
			case 'M':
				CHECK_NULL (vict_obj,
							HMHR ((const struct char_data *) vict_obj));
				break;
			case 's':
				i = HSHR (ch);
				break;
			case 'S':
				CHECK_NULL (vict_obj,
							HSHR ((const struct char_data *) vict_obj));
				break;
			case 'e':
				i = HSSH (ch);
				break;
			case 'E':
				CHECK_NULL (vict_obj,
							HSSH ((const struct char_data *) vict_obj));
				break;
			case 'o':
			...
			case 'P':
				CHECK_NULL (vict_obj,
							OBJS ((const struct obj_data *) vict_obj, to));
				break;
			case 'a':
			...
			case 'A':
				CHECK_NULL (vict_obj,
							SANA ((const struct obj_data *) vict_obj));
				break;
			case 'T':
...
void
circle_sleep (struct timeval *timeout)
{
