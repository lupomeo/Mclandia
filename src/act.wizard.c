...
ACMD (do_send)
{
	...
	if (!GET_SKILL (ch, SKILL_SEND) && !IS_PSIONIC (ch))
	{
		send_to_char ("Non hai poteri telepatici.\r\n", ch);
		return;
	}

	if (!GET_SKILL (ch, SKILL_MESSENGER) && IS_PSIONIC (ch))
	{
		send_to_char ("Non hai poteri psionici.\r\n", ch);
		return;
	}

	if (!*arg)
	{
		send_to_char ("Spedire cosa  a chi?\r\n", ch);
		return;
	}
	...
	/*
	   send_to_char(GET_NAME(ch), vict);
	   send_to_char(" pensa: ", vict);
	   send_to_char(buf, vict);
	   send_to_char("\r\n", vict);
	 */
	if (COLOR_LEV (ch) >= C_CMP)
	{
		sprintf (buf2, "\x1B[1;33m$n pensa: '%s'\x1B[0;0m", buf);
	}
	else
	{
		sprintf (buf2, "$n pensa: '%s'", buf);
	}
	act (buf2, FALSE, ch, 0, vict, TO_VICT);


	if (PRF_FLAGGED (ch, PRF_NOREPEAT))
		send_to_char ("Spedito.\r\n", ch);
	else
	{
		sprintf (buf2, "Mandi '%s' a %s.\r\n", buf, GET_NAME (vict));
		...
		else if ((target_obj = get_obj_vis (ch, mobobjstr, &num)) != NULL)
		{
			if (target_obj->in_room != NOWHERE)
				location = target_obj->in_room;
					 ...
					 && target_obj->carried_by->in_room != NOWHERE)
				location = target_obj->carried_by->in_room;
			else if (target_obj->worn_by
					 && target_obj->worn_by->in_room != NOWHERE)
	...
	for (i = 0; i < NUM_OF_DIRS; i++)
	{
		if (rm->dir_option[i])
		{
			...
			sprintf (buf,
					 ...
					 rm->dir_option[i]->key,
					 rm->dir_option[i]->keyword ? rm->
					 dir_option[i]->keyword : "None", buf2);
			send_to_char (buf, ch);
...
do_stat_object (struct char_data *ch, struct obj_data *j)
{
	...
	case ITEM_MONEY:
		...
		break;
	case ITEM_MISSILE:
		sprintf (buf, "Ammo type:%s Num. units:%d Damage: %dD%d",
				 types_ammo[GET_OBJ_VAL (j, 0)][0], 
                 GET_OBJ_VAL (j, 3),
                 shot_damage[GET_OBJ_VAL (j, 0)][0],
                 shot_damage[GET_OBJ_VAL (j, 0)][1]);
		break;
...
do_stat_character (struct char_data *ch, struct char_data *k)
{
	...
	sprintf (buf,
			 "Hit p.:[%s%d/%d+%d%s]  Mana/RAM p.:[%s%d/%d+%d%s]  Move p.:[%s%d/%d+%d%s]\r\n",
			 ...
			 GET_MAX_MANA (k), mana_gain (k), CCNRM (ch, C_NRM), CCGRN (ch,
																		C_NRM),
			 GET_MOVE (k), GET_MAX_MOVE (k), move_gain (k), CCNRM (ch,
																   C_NRM));
	send_to_char (buf, ch);
	...
	sprintf (buf,
			 ...
			 k->points.damroll, GET_SAVE (k, 0), GET_SAVE (k, 1), GET_SAVE (k,
																			2),
		...
		for (aff = k->affected; aff; aff = aff->next)
		{
			*buf2 = '\0';
			sprintf (buf, "SPL: (%3dhr) %s%-21s%s ", aff->duration + 1,
					 CCCYN (ch, C_NRM), skill_name (aff->type), CCNRM (ch,
																	   C_NRM));
			if (aff->modifier)
			{
...
//    struct char_file_u tmp_store;
	...
	else if (is_abbrev (buf1, "mob"))
	{
		...
		{
			if ((victim = get_char_vis (ch, buf2, NULL, FIND_CHAR_WORLD)) !=
				NULL)
	...
	else
	{
		...
		if ((object = get_object_in_equip_vis (ch, buf1, ch->equipment, &tmp))
			...
			if (
				(object =
				 get_obj_in_list_vis (ch, buf1, &number,
									  ch->carrying)) != NULL)
			do_stat_object (ch, object);
		else if ((victim = get_char_vis (ch, buf1, &number, FIND_CHAR_ROOM))
				 != NULL)
			do_stat_character (ch, victim);
		else
			if (
			...
			do_stat_object (ch, object);
		else if ((victim = get_char_vis (ch, buf1, &number, FIND_CHAR_WORLD))
				 != NULL)
			do_stat_character (ch, victim);
		else if ((object = get_obj_vis (ch, buf1, &number)) != NULL)
			do_stat_object (ch, object);
		else
...
stop_snooping (struct char_data *ch)
{
	...
	else
	{
		...
		log ("(GC) %s stop to snoop %s.",
			 GET_NAME (ch), GET_NAME (ch->desc->snooping->character));
...
ACMD (do_snoop)
{
	...
	else
	{
		...
		log ("(GC) %s is starting to snoop %s.",
...
perform_immort_invis (struct char_data *ch, int level)
{
	...
	for (tch = world[ch->in_room].people; tch; tch = tch->next_in_room)
	{
		...
		if (GET_LEVEL (tch) >= GET_INVIS_LEV (ch) && GET_LEVEL (tch) < level)
			act ("Batti gli occhi e ti accorgi che $n non c'e'.", FALSE, ch,
				 0, tch, TO_VICT);
		if (GET_LEVEL (tch) < GET_INVIS_LEV (ch) && GET_LEVEL (tch) >= level)
			act ("Ti rendi conto che $n e' davanti a te.", FALSE, ch, 0,
				 tch, TO_VICT);
	}
...
void
print_zone_to_buf (char *bufptr, zone_rnum zone)
{
	sprintf (bufptr,
			 ...
			 char *val_arg)
{
	...
	case 2:
		...
		sprintf (output, "%s's title is now: %s", GET_NAME (vict),
				 GET_TITLE (vict));
		break;
	case 3:
