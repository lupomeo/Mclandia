...
extern long max_exp_gain;
extern long max_exp_group_gain;
...
void
gain_exp (struct char_data *ch, long gain)
{
...
void
gain_exp_regardless (struct char_data *ch, long gain)
{
	int is_altered = FALSE;
	int num_levels = 0;
...
long
level_exp_ratio(struct char_data *ch, struct char_data *vict, long exp)
{
    int diff,  result;
    float coeff;
    ...
    if ( diff_level_cap == 1 )
    {
        /* Diff Level Cap */
        /*
        diff = abs(GET_LEVEL (vict) - GET_LEVEL (ch));
        result = 1;

         coeff = ((float)GET_LEVEL (vict)/(float)GET_LEVEL (ch));
        ...
        */
        ...
        coeff = (long)( exp / level_cap(ch) )* abs(diff);
...
long 
group_level_ratio_exp(struct char_data * ch, int group_max_level, long exp)
{
    int diff = 0;
...
long 
group_exp_cap(int num_group_members)
{
    long total_exp = 0;

    switch (num_group_members)
    {
    case 1:
        total_exp =  max_exp_gain;
        break;
    case 2:
    case 3:
    case 4:
    ...
    case 8:
    case 9:
        /* tra 2 e 9 vaglono 250000xp e 900000xp */
        total_exp = ((650/7)*num_group_members+450/7)*1000;
        break;
    default:
        total_exp = max_exp_group_gain;
        break;
    }
    return total_exp;
}
...
/* da ripensare  bene */
long 
modify_bonus_exp( struct char_data *ch, struct char_data *victim, long total)
{
    /*
         ...
         !AFF_FLAGGED(victim, AFF_CHARM) )
    {
        if ( GET_LEVEL (ch) > 10 )
        {
            total = level_exp_ratio(ch, victim, total);
        }
