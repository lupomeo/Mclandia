...
House_save_control (void)
{
	...
	fwrite (house_control, sizeof (struct house_control_rec), num_of_houses,
			fl);
...
hcontrol_pay_house (struct char_data *ch, char *arg)
{
	...
	{
		sprintf (buf, "Payment for house %s collected by %s.", arg,
				 GET_NAME (ch));
...
void
House_list_guests (struct char_data *ch, int i, int quiet)
{
	int j;
	char *temp;
