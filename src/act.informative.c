...
ACMD (do_ps_aux)
{
	const char *format = "%6d %-4s %2d:00  %s\r\n";
	...
	if (ch->affected)
	{
		...
		sprintf (buf, "   PID STAT  TIME  COMMAND\r\n");
		//send_to_char (buf, ch);

		for (aff = ch->affected; aff; aff = aff->next)
		{
			
            if (aff->type < MAX_NON_PSI_SPELL)
            {
				sprintf (prg_name, "%s%-14s%s", CCCYN (ch, C_NRM),
						 skill_name (aff->type), CCNRM (ch, C_NRM));
				time = aff->duration;
                if (aff->duration == 0)
                {
                    sprintf (tbuf, format, aff->type, "S", time, prg_name);
                }
                else
                {
                    sprintf (tbuf, format, aff->type, "R", time, prg_name);
                }
				//send_to_char (buf, ch);
                strcat(buf,tbuf);
                pid++;
        ...
        if ( pid == 0 )
        {
            send_to_char ("Non ci sono processi in esecuzione.\r\n", ch);
        }
        else
        {
            send_to_char (buf, ch);
...
ACMD (do_psychic)
{
	...
	if (ch->affected)
	{
		...
		//sprintf (buf, "Active Psi Spells Time\r\n");
        sprintf (buf, "Active Psi Spells:\r\n");
		//send_to_char (buf, ch);

		for (aff = ch->affected; aff; aff = aff->next)
		{
			
            if (aff->type > MAX_NON_PSI_SPELL && aff->type < MAX_SPELLS)
            {
				sprintf (prg_name, "%s%-14s%s", CCCYN (ch, C_NRM),
						 skill_name (aff->type), CCNRM (ch, C_NRM));
        ...
        if ( pid == 0 )
        {
            send_to_char ("Non hai poteri psi attivi.\r\n", ch);
        }
        else
        {
            send_to_char (buf, ch);
