...
int castle_twin_proc (struct char_data *ch, int cmd, char *arg, int ctlnum,
					  char *twinname);
 ...
 */
void
assign_kings_castle (void)
{
	int check;

#define CASTLE_DISABLED 1

#ifdef CASTLE_DISABLED
 ...
 */
struct char_data *
find_npc_by_name (struct char_data *chAtChar, const char *pszName, int iLen)
{
	struct char_data *ch;
 ...
 */
int
block_way (struct char_data *ch, int cmd, char *arg, room_rnum iIn_room,
		   int iProhibited_direction)
{
	...
	if (!member_of_staff (ch))
		act ("The guard roars at $n and pushes $m back.", FALSE, ch, 0, 0,
			 TO_ROOM);
...
fry_victim (struct char_data *ch)
{
	...
	case 7:
		...
		act ("$n looks deeply into the eyes of $N.", 1, ch, 0, tch,
			 TO_NOTVICT);
	...
	if (!move)
	{
		if (time_info.hours == 8 && ch->in_room == R_ROOM (Z_KINGS_C, 51))
		{
			...
			index = 0;
				 ...
				 && ch->in_room == R_ROOM (Z_KINGS_C, 17))
		{
			move = TRUE;
			path = bedroom_path;
			index = 0;
...
SPECIAL (training_master)
{
	...
	{
	case 0:
		act ("$n hits $N on $s head with a powerful blow.", FALSE, pupil1, 0,
			 pupil2, TO_NOTVICT);
		act ("You hit $N on $s head with a powerful blow.", FALSE, pupil1, 0,
			 pupil2, TO_CHAR);
		act ("$n hits you on your head with a powerful blow.", FALSE, pupil1,
			 0, pupil2, TO_VICT);
		break;

	case 1:
		act ("$n hits $N in $s chest with a thrust.", FALSE, pupil1, 0,
			 pupil2, TO_NOTVICT);
		act ("You manage to thrust $N in the chest.", FALSE, pupil1, 0,
			 pupil2, TO_CHAR);
		act ("$n manages to thrust you in your chest.", FALSE, pupil1, 0,
			 pupil2, TO_VICT);
		break;
	...
	case 2:
		...
		act ("$N bows before $n.", FALSE, pupil1, 0, pupil2, TO_NOTVICT);
		act ("You bow before $N, who returns your gesture.", FALSE, pupil1, 0,
			 pupil2, TO_CHAR);
		act ("You bow before $n, who returns your gesture.", FALSE, pupil1, 0,
			 pupil2, TO_VICT);
		break;
	...
	case 3:
		act ("$N yells at $n, as he fumbles and drops $s sword.", FALSE,
			 pupil1, 0, ch, TO_NOTVICT);
		act ("$n quickly picks up $s weapon.", FALSE, pupil1, 0, 0, TO_ROOM);
		act ("$N yells at you, as you fumble, losing your weapon.", FALSE,
			 pupil1, 0, ch, TO_CHAR);
		send_to_char ("You quickly pick up your weapon again.\r\n", pupil1);
		act ("You yell at $n, as he fumbles, losing $s weapon.", FALSE,
			 pupil1, 0, ch, TO_VICT);
		break;

	case 4:
		act ("$N tricks $n, and slashes him across the back.", FALSE, pupil1,
			 0, pupil2, TO_NOTVICT);
		act ("$N tricks you, and slashes you across your back.", FALSE,
			 pupil1, 0, pupil2, TO_CHAR);
		act ("You trick $n, and quickly slash him across $s back.", FALSE,
			 pupil1, 0, pupil2, TO_VICT);
		break;

	case 5:
		act ("$n lunges a blow at $N but $N parries skillfully.", FALSE,
			 pupil1, 0, pupil2, TO_NOTVICT);
		act ("You lunge a blow at $N but $E parries skillfully.", FALSE,
			 pupil1, 0, pupil2, TO_CHAR);
		act ("$n lunges a blow at you, but you skillfully parry it.", FALSE,
			 pupil1, 0, pupil2, TO_VICT);
		break;

	case 6:
		act ("$n clumsily tries to kick $N, but misses.", FALSE, pupil1, 0,
			 pupil2, TO_NOTVICT);
		act ("You clumsily miss $N with your poor excuse for a kick.", FALSE,
			 pupil1, 0, pupil2, TO_CHAR);
		act ("$n fails an unusually clumsy attempt at kicking you.", FALSE,
			 pupil1, 0, pupil2, TO_VICT);
		break;

	default:
		send_to_char ("You show your pupils an advanced technique.\r\n", ch);
		act ("$n shows $s pupils an advanced technique.", FALSE, ch, 0, 0,
			 TO_ROOM);
		break;
	}
 ...
 */
int
castle_twin_proc (struct char_data *ch, int cmd, char *arg, int ctlnum,
				  char *twinname)
{
 ...
 */
int
castle_cleaner (struct char_data *ch, int cmd, int gripe)
{
	struct obj_data *i;
