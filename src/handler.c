...
isname (const char *str, const char *namelist)
{
	...
	for (curtok = strtok (newlist, WHITESPACE); curtok;
		 curtok = strtok (NULL, WHITESPACE))
	{
		//if(curtok && is_apart(str, curtok))
		if (curtok && is_abbrev (str, curtok))	//in Jvrgen's code 
		{
...
void
affect_modify (struct char_data *ch, byte loc, sbyte mod,
...
char_from_room (struct char_data *ch)
{
	...
	{
		log ("SYSERR: NULL character or NOWHERE in %s, char_from_room",
			 __FILE__);
		exit (1);
	}
...
obj_to_char (struct obj_data *object, struct char_data *ch)
{
	if (object && ch)
	{
		...
		log ("SYSERR: NULL obj (%p) or char (%p) passed to obj_to_char.",
			 object, ch);
}
...
int
invalid_align (struct char_data *ch, struct obj_data *obj)
{
...
unequip_char (struct char_data *ch, int pos)
{
	...
	if (ch->in_room != NOWHERE)
	{
		...
		log ("SYSERR: ch->in_room = NOWHERE when unequipping char %s.",
			 GET_NAME (ch));
...
struct char_data *
get_char_room (char *name, int *number, room_rnum room)
{
	struct char_data *i;
	int num;
...
obj_to_room (struct obj_data *object, room_rnum room)
{
	if (!object || room < 0 || room > top_of_world)
		log
			("SYSERR: Illegal value(s) passed to obj_to_room. (Room #%d/%d, obj %p)",
...
obj_from_room (struct obj_data *object)
{
	...
	{
		log
			("SYSERR: NULL object (%p) or obj not in a room (%d) passed to obj_from_room",
...
obj_to_obj (struct obj_data *obj, struct obj_data *obj_to)
{
	...
	{
		log
			("SYSERR: NULL object (%p) or same source (%p) and target (%p) obj passed to obj_to_obj.",
			 obj, obj, obj_to);
		return;
	}
...
obj_from_obj (struct obj_data *obj)
{
	...
	if (obj->in_obj == NULL)
	{
		log ("SYSERR: (%s): trying to illegally extract obj from obj.",
			 __FILE__);
		return;
	}
	obj_from = obj->in_obj;
		...
		if (GET_OBJ_TYPE (GET_EQ (ch, WEAR_LIGHT)) == ITEM_LIGHT)
			if (GET_OBJ_VAL (GET_EQ (ch, WEAR_LIGHT), 2) > 0)
			{
				...
				if (i == 1)
				{
					...
					act ("$n's light begins to flicker and fade.", FALSE, ch,
						 0, 0, TO_ROOM);
				}
				else if (i == 0)
				{
		...
		else
		{
			...
			for (d = descriptor_list; d; d = d->next)
			{
				if (d == ch->desc)
					continue;
				if (d->character
					&& GET_IDNUM (ch) == GET_IDNUM (d->character))
					STATE (d) = CON_CLOSE;
			}
...
struct char_data *
get_player_vis (struct char_data *ch, char *name, int *number, int inroom)
{
	struct char_data *i;
	int num;
...
struct char_data *
get_char_room_vis (struct char_data *ch, char *name, int *number)
{
	struct char_data *i;
	int num;
	...
	for (i = world[ch->in_room].people; i && *number; i = i->next_in_room)
	{
		if (isname (name, i->player.name))
		{
			if (CAN_SEE (ch, i))
...
struct char_data *
get_char_world_vis (struct char_data *ch, char *name, int *number)
{
	struct char_data *i;
	int num;
...
struct char_data *
get_char_vis (struct char_data *ch, char *name, int *number, int where)
{
...
struct obj_data *
get_obj_in_list_vis (struct char_data *ch, char *name, int *number,
...
struct obj_data *
get_obj_vis (struct char_data *ch, char *name, int *number)
{
	struct obj_data *i;
	int num;
...
/* Hm, number deficient... */
struct obj_data *
get_object_in_equip_vis (struct char_data *ch, char *arg,
...
money_desc (int amount)
{
	...
	money_table[] =
	{
		...
		,
		{
		10, "una piccolo mucchietto di monete d'oro"}
		,
		{
		20, "una manciata di monete d'oro"}
		,
		{
		75, "una mucchietto di monete d'oro"}
		,
		{
		200, "una piccola pila di monete d'oro"}
		,
		{
		1000, "una pila di monete d'oro"}
		,
		{
		5000, "una grande pila di monete d'oro"}
		,
		{
		10000, "un grande cumulo di monete d'oro"}
		,
		{
		20000, "una collina di monete d'oro"}
		,
		{
		75000, "una enorme collina di monete d'oro"}
		,
		{
		150000, "una montagnetta di monete d'oro"}
		,
		{
		250000, "una montagna di monete d'oro"}
		,
		{
		500000, "una enorme montagna di monete d'oro"}
		,
		{
		1000000, "una smisurata montagna di monete d'oro"}
		,
		{
		0, NULL}
	,};
	...
	return ("una smisurata anormale montagna di monete d'oro");
}
...
create_money (int amount)
{
	...
	if (amount == 1)
	{
		obj->name = str_dup ("coin gold moneta");
		obj->short_description = str_dup ("una moneta d'oro");
		obj->description =
			str_dup ("Una sola miserabile monetad'oro giace qui.");
		new_descr->keyword = str_dup ("coin gold moneta");
		new_descr->description =
			str_dup ("E' solo una miserabile moneta d'oro.");
	}
	else
	{
		obj->name = str_dup ("coins gold monete");
		obj->short_description = str_dup (money_desc (amount));
		sprintf (buf, "%s giaciono qui.", money_desc (amount));
		obj->description = str_dup (CAP (buf));

		new_descr->keyword = str_dup ("coins gold monete");
		if (amount < 10)
		{
		...
		{
			sprintf (buf, "Ci sono poco piu di %d monete.",
					 10 * (amount / 10));
			new_descr->description = str_dup (buf);
		}
		else if (amount < 1000)
		{
			sprintf (buf, "Sembra circa %d monete.", 100 * (amount / 100));
			new_descr->description = str_dup (buf);
		}
		else if (amount < 100000)
		{
			sprintf (buf, "Credi che ci siano, circa, %d monete.",
					 1000 * ((amount / 1000) + number (0, (amount / 1000))));
			new_descr->description = str_dup (buf);
		}
		else
			new_descr->description =
				str_dup ("Ci sono veramente TANTE monete.");
	}
 ...
 */
int
generic_find (char *arg, bitvector_t bitvector, struct char_data *ch,
