...
int fail_check_level_diff(int levch, int levvict);
...
ACMD (do_backstab)
{
	...
	if (MOB_FLAGGED (vict, MOB_AWARE) && AWAKE (vict) && fail_check_level_diff(GET_LEVEL(ch),GET_LEVEL(vict)) )
	{
...
ACMD (do_bash)
{
	...
	if (MOB_FLAGGED (vict, MOB_NOBASH) && fail_check_level_diff(GET_LEVEL(ch),GET_LEVEL(vict)) )
	{
...
int fail_check_level_diff(int ch_level, int vict_level)
{
	int percent;
	int prob;
	...
	else if ( (ch_level - vict_level) == 10)
	{
		percent = 75;
	}
	else if ( (ch_level - vict_level) == 20)
	{
		percent = 50;
	}
	else 
