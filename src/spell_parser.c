...
#define USE_BINARY_TEXT
//#undef USE_BINARY_TEXT
#ifdef USE_BINARY_TEXT
    {" ", "0100000"},
	{"a", "1100001"}, 
    {"b", "1100010"}, 
    {"c", "1100011"}, 
...
mag_manacost (struct char_data *ch, int spellnum)
{
	return MAX (SINFO.mana_max - (SINFO.mana_change *
								  (GET_LEVEL (ch) -
								   SINFO.min_level[(int) GET_CLASS (ch)])),
				SINFO.mana_min);
}
			...
			struct obj_data *ovict, int spellnum, int level, int casttype)
{
	...
	if (ROOM_FLAGGED (IN_ROOM (caster), ROOM_NOMAGIC))
	{
		...
		act ("$n's magic fizzles out and dies.", FALSE, caster, 0, 0,
			 TO_ROOM);
		return (0);
	}
	...
	if (IS_SET (SINFO.routines, MAG_MANUAL))
		...
		case SPELL_PSI_HYPNOSIS:
			MANUAL_SPELL (spell_charm);
			break;
		case SPELL_PSI_SUMMON:
			MANUAL_SPELL (spell_summon);
			break;
		case SPELL_PSI_TELEPORT:
			MANUAL_SPELL (spell_teleport);
			break;
		case SPELL_PSI_MIND_OVER_BODY:
			MANUAL_SPELL (spell_mind_over_body);
			break;
		case SPELL_PSI_CANIBALIZE:
			MANUAL_SPELL (spell_canibalize);
			break;
		}
...
mag_objectmagic (struct char_data *ch, struct obj_data *obj, char *argument)
{
	...
	case ITEM_STAFF:
		...
		else
		{
			...
			k =
				GET_OBJ_VAL (obj, 0) ? GET_OBJ_VAL (obj,
													0) : DEFAULT_STAFF_LVL;
				...
				(GET_OBJ_VAL (obj, 3), MAG_MASSES | MAG_AREAS))
			{
				for (i = 0, tch = world[IN_ROOM (ch)].people; tch;
					...
					i++;
				while (i-- > 0)
					call_magic (ch, NULL, NULL, GET_OBJ_VAL (obj, 3), k,
								CAST_STAFF);
			}
			else
			{
				for (tch = world[IN_ROOM (ch)].people; tch; tch = next_tch)
				{
					...
					if (ch != tch)
						call_magic (ch, tch, NULL, GET_OBJ_VAL (obj, 3), k,
									CAST_STAFF);
				}
			}
		}
	...
	case ITEM_SCROLL:
		...
		act ("Connetti $p alla tua interfaccia.", TRUE, ch, obj, 0, TO_CHAR);
		if (obj->action_description)
			act (obj->action_description, FALSE, ch, obj, NULL, TO_ROOM);
		else
			act ("$n connette $p alla sua interfaccia.", FALSE, ch, obj, NULL,
				 TO_ROOM);
...
ACMD (do_cast)
{
	...
	/* Sei psionico? Vattene! */
	if (IS_PSIONIC (ch) && (GET_LEVEL (ch) != LVL_GOD))
		return;
	...
	if (s == NULL)
	{
		send_to_char ("Eseguire cosa e dove?\r\n", ch);
		return;
	}
	s = strtok (NULL, "'");
	if (s == NULL)
	{
		send_to_char
			("I nomi dei programmi devono essere inglobati dai simboli: '\r\n",
			 ch);
		return;
	}
	t = strtok (NULL, "\0");
	...
	{
		send_to_char ("Eseguire cosa?!?\r\n", ch);
		return;
	}
	if (GET_LEVEL (ch) < SINFO.min_level[(int) GET_CLASS (ch)])
	{
		send_to_char
			("Non hai i diritti di esecuzione per questo programma!\r\n", ch);
		return;
	}
	if (GET_SKILL (ch, spellnum) == 0)
	{
		send_to_char ("Non sai come usarlo questo programma.\r\n", ch);
		return;
	}
	/* Find the target */
	if (t != NULL)
	...
	else if (t != NULL && *t)
	{
		...
		{
			if ((tobj = get_obj_in_list_vis (ch, t, NULL, ch->carrying)) !=
				NULL)
			{
				target = TRUE;
			}
	...
	else						/* if target string is empty */
	{
		...
		if (!target)
		{
			sprintf (buf, "Su %s pensi di eseguire il tuo programma?\r\n",
					 IS_SET (SINFO.targets,
							 TAR_OBJ_ROOM | TAR_OBJ_INV | TAR_OBJ_WORLD |
							 TAR_OBJ_EQUIP) ? "cosa" : "chi");
			send_to_char (buf, ch);
			return;
		}
	}
	...
	{
		send_to_char
			("Non puoi eseguirlo su se stesso, puo' essere dannoso alla tua salute!\r\n",
			 ch);
		return;
	}
	if (!target)
	{
		send_to_char ("Non trovo il bersaglio del tuo programma!\r\n", ch);
		return;
	}
	...
	{
		send_to_char
			("Non hai abbastanza RAM per eseguire il tuo programma!\r\n", ch);
		return;
	}
	...
	if (number (0, 101) > GET_SKILL (ch, spellnum))
	{
		...
		if (!tch || !skill_message (0, ch, tch, spellnum))
		{
            ...
            learned_from_mistake(ch, spellnum, 0, 95);
		}
		if (mana > 0)
		{
			GET_MANA (ch) =
...
/* do_psi per castare le spell degli psionici */
ACMD (do_psi)
{
	struct char_data *tch = NULL;
	struct obj_data *tobj = NULL;
	...
	/* se non e' psionico vai via! */
	if ((GET_CLASS (ch) != CLASS_PSIONIC) && (GET_LEVEL (ch) != LVL_GOD))
	{
		return;
	}
	...
	if (s == NULL)
	{
		send_to_char ("Usare i poteri psichichi a cosa e dove?\r\n", ch);
		return;
	}
	s = strtok (NULL, "'");
	if (s == NULL)
	{
		send_to_char
			("I nomi dei poteri psichici vanno seguiti dai simboli: '\r\n",
			 ch);
		return;
	}
	t = strtok (NULL, "\0");
	...
	if (GET_LEVEL (ch) < SINFO.min_level[(int) GET_CLASS (ch)])
	{
		send_to_char ("Non lo conosci!\r\n", ch);
		return;
	}

	if (GET_SKILL (ch, spellnum) == 0)
	{
		send_to_char ("Non sei addestrato a farlo.\r\n", ch);
		return;
	}
	...
	else						/* if target string is empty */
	{
		...
		if (!target)
		{
			sprintf (buf, "A %s devo usare il potere psionico?\r\n",
					 IS_SET (SINFO.targets,
							 TAR_OBJ_ROOM | TAR_OBJ_INV | TAR_OBJ_WORLD |
							 TAR_OBJ_EQUIP) ? "cosa" : "chi");
			send_to_char (buf, ch);
	...
	{
		send_to_char
			("Non puoi usare su se stesso, potrebbe essere pericoloso per la tua vita!\r\n",
			 ch);
		return;
	}
	if (!target)
	...
	if ((mana > 0) && (GET_MANA (ch) < mana) && (GET_LEVEL (ch) < LVL_IMMORT))
	{
		send_to_char ("Non hai RAM sufficiente per usare il tuo potere!\r\n",
					  ch);
		return;
	}
...
void
spello (int spl, const char *name, int max_mana, int min_mana,
...
#define psyco(spell, name, max_mana, min_mana, mana_change, minpos, targets, violent, routines, wearoff) \
    spello(spell, name, max_mana, min_mana,	mana_change, minpos, targets, violent, routines, wearoff);
