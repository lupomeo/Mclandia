...
extern int level_exp (int chclass, int level);	//from class.c
...
int degree_sum (int base, int inc);
int check_degree (int test, int check, int interval);
int check_in_arc (int test, int start, int end);
void learned_from_mistake (struct char_data *ch, int skill, int silent,
						   int max);
...
long level_exp_ratio(struct char_data *ch, struct char_data *vict,long exp);
long group_level_ratio_exp(struct char_data * ch, int group_max_level, long exp);
long group_exp_cap(int num_group_members);
long modify_bonus_exp( struct char_data *ch, struct char_data *victim, long total);
...
/* See also: ANA, SANA */
#define AN(string) (strchr("aeiouAEIOU", *string) ? "un'" : "un")
...
#define IS_AUTO_FIREWEAPON(obj) (IS_SET(GET_OBJ_EXTRA(obj), ITEM_AUTOMATIC_FIREWEAPON))
...
#define IN_ZONE(ch) (zone_table[(world[((ch)->in_room)].zone)].number)
#define IN_SECTOR(ch) (world[((ch)->in_room)].sector_type)
...
#define GET_MIN_XP_LEVEL(ch) (level_exp(GET_CLASS(ch),GET_REAL_LEVEL(ch)))
...
#define ANA(obj) (strchr("aeiouAEIOU", *(obj)->name) ? "un'" : "un")
#define SANA(obj) (strchr("aeiouAEIOU", *(obj)->name) ? "un'" : "un")
...
#define PERS(ch, vict)   (CAN_SEE(vict, ch) ? GET_NAME(ch) : "qualcuno")
...
#define IS_MARTIAL(ch)		(!IS_NPC(ch) && \
				(GET_CLASS(ch) == CLASS_MARTIAL))
#define IS_LINKER(ch)		(!IS_NPC(ch) && \
				(GET_CLASS(ch) == CLASS_LINKER))
#define IS_PSIONIC(ch)		(!IS_NPC(ch) && \
				(GET_CLASS(ch) == CLASS_PSIONIC))
#define IS_UMANO(ch)            (!IS_NPC(ch) && \
				(GET_RACE(ch) == RACE_UMANO))

#define IS_CYBORG(ch)              (!IS_NPC(ch) && \
				(GET_RACE(ch) == RACE_CYBORG))

#define IS_MOTOCICLISTA(ch)            (!IS_NPC(ch) && \
				(GET_RACE(ch) == RACE_MOTOCICLISTA))

#define IS_ALIENO(ch)            (!IS_NPC(ch) && \
				(GET_RACE(ch) == RACE_ALIENO))
