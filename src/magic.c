 ...
 */
int
mag_savingthrow (struct char_data *ch, int type, int modifier)
{
			...
			else
			{
				if ((af->type > 0) && (af->type <= MAX_SPELLS))
                {
						...
						(af->next->duration > 0))
                    {
						...
						{
							send_to_char (spell_info[af->type].wear_off_msg,
										  i);
							send_to_char ("\r\n", i);
						}
                    }
			...
			int spellnum, int savetype)
{
	...
	case SPELL_CHILL_TOUCH:	/* chill touch also has an affect */
		if (IS_MAGIC_USER (ch))
			dam = dice (1, 8) * GET_LEVEL (ch) / 2;
		else
			dam = dice (1, 6) * GET_LEVEL (ch) / 2;
		break;

	case SPELL_BURNING_HANDS:
		if (IS_MAGIC_USER (ch))
			dam = dice (3, 8) * GET_LEVEL (ch) / 2;
		else
			dam = dice (3, 6) * GET_LEVEL (ch) / 2;
		break;

	case SPELL_SHOCKING_GRASP:
		if (IS_MAGIC_USER (ch))
			dam = dice (5, 8) * GET_LEVEL (ch) / 3;
		else
			dam = dice (5, 6) * GET_LEVEL (ch) / 3;
		break;

	case SPELL_LIGHTNING_BOLT:
		if (IS_MAGIC_USER (ch))
			dam = dice (7, 8) * GET_LEVEL (ch) / 4;
		else
			dam = dice (7, 6) * GET_LEVEL (ch) / 4;
		break;

	case SPELL_COLOR_SPRAY:
		if (IS_MAGIC_USER (ch))
			dam = dice (9, 8) + GET_LEVEL (ch);
		else
			dam = dice (9, 6) + GET_LEVEL (ch);
		break;

	case SPELL_FIREBALL:
		if (IS_MAGIC_USER (ch))
			dam = dice (11, 8) + GET_LEVEL (ch);
		else
			dam = dice (11, 6) + GET_LEVEL (ch);
		break;

		/* mostly psyonic */
	case SPELL_PSI_BLAST:
		dam = dice (1, 4) * GET_LEVEL (ch) / 2;
		break;

	case SPELL_PSI_BURN:
		dam = dice (2, 8) + GET_LEVEL (ch);
		break;

	case SPELL_PSI_FLAME:
		dam = dice (5, 8) + GET_LEVEL (ch) / 3;
		break;
			 ...
			 int spellnum, int savetype)
{
	...
	case SPELL_ARMOR:
	case SPELL_PSI_SHIELD:
		af[0].location = APPLY_AC;
		af[0].modifier = -20;
		af[0].duration = 24 + GET_LEVEL (ch) / 4;
		accum_duration = TRUE;
		to_vict = "Senti che qualcosa ti sta proteggendo.";
		break;

	case SPELL_BLESS:
		af[0].location = APPLY_HITROLL;
		af[0].modifier = 2;
		af[0].duration = 6 + GET_LEVEL (ch) / 4;

		af[1].location = APPLY_SAVING_SPELL;
		af[1].modifier = -1;
		af[1].duration = 6 + GET_LEVEL (ch) / 4;

		accum_duration = TRUE;
		to_vict = "Ti senti piu' in forma.";
		break;
	...
	case SPELL_BLINDNESS:
		...
		to_room = "$n sembra essere stato accecato!";
	...
	case SPELL_PSI_AURA:
		...
		accum_duration = TRUE;
		to_vict = "I tuoi occhi brillano brevemente.";
		to_room = "Gli occhi di $n brillare brevemente";
		break;

	case SPELL_DETECT_INVIS:
	case SPELL_PSI_SIGHT:
		af[0].duration = 12 + GET_LEVEL (ch) / 2;
	...
	case SPELL_PSI_NIGHTVISION:
		af[0].duration = 12 + GET_LEVEL (ch) / 2;
		af[0].bitvector = AFF_INFRAVISION;
		accum_duration = TRUE;
		to_vict = "I tuoi occhi brillano di rosso.";
		to_room = "Gli occhi di $n brillano di rosso.";
		break;
	...
	case SPELL_POISON:
		...
		af[0].modifier = -2 + number (1, GET_LEVEL (ch) / 2);
		af[0].bitvector = AFF_POISON;
		to_vict = "Ti senti male. Sei stato infettato.";
		to_room = "$n appare molto malato!";
		break;
	...
	case SPELL_PROT_FROM_EVIL:
		...
		accum_duration = TRUE;
		to_vict = "Ti senti invulnerabile!";
		break;
	...
	case SPELL_SLEEP:
		...
		{
			send_to_char ("Ti senti veramente assonnato...  Zzzz......\r\n",
						  victim);
			act ("$n crolla a terra dal sonno.", TRUE, victim, 0, 0, TO_ROOM);
			GET_POS (victim) = POS_SLEEPING;
		}
		break;

	case SPELL_STRENGTH:
	case SPELL_PSI_STRENGHT:
	...
	case SPELL_FLY:
	case SPELL_PSI_LEVITATION:
		af[0].duration = 24;
		af[0].bitvector = AFF_FLY;
		accum_duration = TRUE;
		to_vict = "Ti alzi lentamente dal suolo ed inizi a volare.";
		break;
	...
	for (i = 0; i < MAX_SPELL_AFFECTS; i++)
	{
		...
		{
			affect_join (victim, af + i, accum_duration, FALSE, accum_affect,
						 FALSE);
		}
	}
...
mag_areas (int level, struct char_data *ch, int spellnum, int savetype)
{
    ...
    area_damage(ch,dam,SPELL_EARTHQUAKE);
			...
			int spellnum, int savetype)
{
	...
	case SPELL_HEAL:
		hit = 100 + dice (4, 8);
		send_to_char ("Una sensazione di calore invade il tuo corpo.\r\n",
					  victim);
		break;
	}
			   ...
			   int spellnum, int type)
{
	...
	case SPELL_HEAL:
		...
		to_vict = "Ti torna la vista!";
		to_room = "Gli occhi di $n non sembrano piu' ciechi.";
		break;
