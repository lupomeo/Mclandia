...
int
Board_write_message (int board_type, struct char_data *ch, char *arg,
					 struct obj_data *board)
{
	...
	*(tmstr + strlen (tmstr) - 1) = '\0';
	...
	send_to_char
		("Scrivi il tuo messaggio.  Termina con un @ su una nuova linea.\r\n\r\n",
		 ch);
...
int
Board_show_board (int board_type, struct char_data *ch, char *arg,
				  struct obj_data *board)
{
	int i;
	...
	strcpy (buf,
			"Questa e' la bacheca.  Usa: READ/REMOVE <messg #>, WRITE <header>.\r\n"
			"Dovrai guardare la bacheca per salvare il tuo messaggio.\r\n");
...
#endif
		{
			if (MSG_HEADING (board_type, i))
				...
				sprintf (buf + strlen (buf), "%-2d : %s\r\n",
						 num_of_msgs[board_type] - i, MSG_HEADING (board_type,
																   i));
#else
				sprintf (buf + strlen (buf), "%-2d : %s\r\n", i + 1,
...
int
Board_display_msg (int board_type, struct char_data *ch, char *arg,
				   struct obj_data *board)
{
	...
	{
		send_to_char
			("Quel messaggio esiste solo nella tua immaginazione.\r\n", ch);
		return (1);
	}
#if NEWEST_AT_TOP
...
int
Board_remove_msg (int board_type, struct char_data *ch, char *arg,
				  struct obj_data *board)
{
